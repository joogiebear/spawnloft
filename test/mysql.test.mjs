...
test('managed engines are explicit per platform and Mac archives are pinned by architecture', () => {
  ...
  assert.equal(services.canManage('garnet', 'darwin'), false)
  assert.equal(services.canManage('mysql', 'win32'), false)
  assert.equal(services.canManage('mariadb', 'win32'), true)
  ...
  for (const arch of ['arm64', 'x64']) {
    ...
    const archive = mysql.archiveFor(mysql.VERSION, arch)
  ...
  assert.throws(() => mysql.assertSupported({ platform: 'win32', arch: 'x64' }), /requires.*Mac/)
})
...
test('configuration isolates TCP and sockets, disables X Protocol, and quotes paths with spaces', () => {
  ...
  const text = mysql.iniFor(inst, '/tmp/private/mysql.sock')
